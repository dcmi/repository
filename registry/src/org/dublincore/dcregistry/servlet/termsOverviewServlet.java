...
public class termsOverviewServlet extends registryServlet  {
	...
	public void doPost (HttpServletRequest request, HttpServletResponse response)
		...
		try {		
			...
			for (int i=0; i < rList.size(); i++) {
				...
				RDFNode o1 = rdbModel.getResource(Sr1);
				Resource r1 = null;
				ResIterator ri2 = rdbModel.listSubjectsWithProperty(RDFS.subPropertyOf, o1);
				if (ri2.hasNext())
			...
			summaryModel.close();
			transform(response, caw, termsOverview_xsl);
	...
	protected void setXSLTParms (Transformer transformer) {
		try {
			...
			transformer.setParameter("reqType", "termsOverview");
			...
			if (expand != null)
				transformer.setParameter("expand", expand);
