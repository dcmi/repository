...
public class elementsServlet extends registryServlet  {
	...
	public void doPost (HttpServletRequest request, HttpServletResponse response)
		...
		try {		
			...
			Bag refs = summaryModel.createBag("http://dublincore.org/dcregistry/elements/");
			int itemCount = 0;
			...
			summaryModel.close();
			transform(response, caw, elements_xsl);
	...
	/*
	protected Model getRefinements (Resource r1, Model rdbModel) 
		throws ServletException, IOException {
		...
		try {
			...
			while (ri.hasNext()) {
				...
				String termname = term.substring(term.lastIndexOf("/")+1);
				refList.add(termname + "*" + term);        		 	
			}
			
			if (refList.size() > 0) {
				Collections.sort(refList);
				Bag erefs = refinementModel.createBag(r1.toString());
				int itemCount = 0;
				for (int i=0; i < refList.size(); i++) {
					String s = (String) refList.get(i);
					Resource r2 = tempModel.createResource(s.substring(s.lastIndexOf("*")+1));
					erefs.add(r2);
					itemCount++;
				}
			}
	...
	protected void setXSLTParms (Transformer transformer) {
		try {
			...
			transformer.setParameter("reqType", "elements");
