...
<xsl:template match="rdf:Description">
	...
	<xsl:value-of select="concat('/dcregistry/adminResourceManagerServlet?reqType=resource', '&amp;', 'item=', $encoded_res, '&amp;', 'itemType=', $itemType)"/>
	</xsl:attribute>
	<xsl:value-of select="$res"/></a>
	</th></tr>
	
	<!-- <xsl:for-each select="descendant::*[not(local-name() = 'relation')]"> -->
	<xsl:for-each select="descendant::*[(local-name() = 'defines')]">
	...
	<form action="/dcregistry/adminResourceManagerServlet" method="post">
		<xsl:choose>
			<xsl:when test="$itemType='collection'">
				<input type="hidden" name="item" value="{@rdf:about}"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<input type="submit" name="reqType" value="Add New Term"/>
	</form>
	</td></tr>
...
<xsl:template name="print-property-list">
	...
	<tr><td>
	<a><xsl:attribute name="HREF">
	<xsl:value-of select="concat('/dcregistry/adminResourceManagerServlet?reqType=resource', '&amp;', 'item=', $encoded_this_res, '&amp;', 'itemType=', $itemType)"/>
	</xsl:attribute><xsl:value-of select="$this_res"/></a>
	</td></tr>
...
<xsl:template name="emptyCollection">
	...
	<tr><td>This collection does not currently contain any resources.</td></tr>	
